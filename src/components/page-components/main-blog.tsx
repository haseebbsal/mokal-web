...
export default async function MainBlog({ header: headerr, blogs: blogss }: BlogProps) {
  ...
  ...  
    ...
    const header: [{ Value: string, Highlight: boolean }] = headerr ? headerr : fetchData.header
    const blogs: [{ _id: string, title: string, _createdAt: string, imageUrl: string }] = blogss ? blogss : fetchData.header
    return (
