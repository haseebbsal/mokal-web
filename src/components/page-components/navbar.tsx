        ...
        <>
            <div className="flex flex-col gap-4">
                <div className="bg-nav">
                    {!configData.prod && <Marquee className="border-b-2 pb-2 border-gray-300">
                        <p >All features of the website are inactive as the website is still under development for a full scale launch THIS IS JUST A PREVIEW of what to EXPECT in the FUTURE</p>
                    </Marquee>}
                        ...
                        <NavbarContent className=" flex w-full" >
                            <div className="flex w-full sm:gap-16 gap-8 sm:justify-start justify-between items-center">
                                <div className="flex sm:gap-4 gap-2">
                                    ...
                                    {configData.countries?.slice(0, 3).map((e, index) => <p key={e.name} className={`${index != 0 ? "border-l-2" : "sm:border-l-2"} text-xs sm:text-sm pl-2 border-base-blue`}>{e.name}</p>)}
                                </div>
                                <div className="flex gap-2 sm:text-sm text-xs items-center">
                                    <IoIosCall  />
