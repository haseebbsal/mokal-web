...
interface CardsProps {
    ...
    cards: [{ imageUrl: string, description: string, heading: string, buttons?: [{ withBackground: boolean, url: { current: string }, text: string }] }]
    description: string
    buttons?: [{ withBackground: boolean, url: { current: string }, text: string }]
}

export default async function CardsContent({ header, cards, variation, description, buttons }: CardsProps) {
    ...
    return (
        <>
                ...
                <div className="sm:w-[80%] m-auto flex flex-col  gap-4 items-center">
                    ...
                    <div className="flex gap-4 flex-wrap w-full">
                        {cards.map((e, index) => {
                            if (index == 0) {
                                return (
                                    <div key={e.heading} className={` bg-base-lightBlue  relative p-8 overflow-hidden  rounded-xl flex-[1_1_15rem]`}>
                                        <div className="flex flex-col gap-2 relative z-30">
                                            <h1 className="font-bold text-base-blue sm:text-2xl text-lg" >{e.heading}</h1>
                                            <p className="text-text-lighterDarkBlue">{e.description}</p>
                                            <Image src={e.imageUrl} alt="shipment1" width={500} className="w-full sm:h-[15rem]" height={500} />
                                        </div>
                            ...
                            if (index == 1) {
                                return (
                                    <div key={e.heading} className=" bg-linearPurple  relative p-8 overflow-hidden  rounded-xl flex-[1_1_15rem]">
                                        <div className="flex flex-col gap-2 relative z-30">
                                            <h1 className="font-bold text-base-purple sm:text-2xl text-lg ">{e.heading}</h1>
                                            <p className="text-base-purple">{e.description}</p>
                                            <Image src={e.imageUrl} alt="shipment2" width={500} className="w-full sm:h-[15rem]" height={500} />
                                        </div>
                                        <LinearGradientCircle />
                                    </div>
                                )
                            ...
                            if (index == 2) {
                                return (
                                    <div key={e.heading} className=" bg-linearBlue p-8 flex sm:flex-row flex-col sm:gap-8 gap-4 items-center rounded-xl w-full">
                                        <div className="flex flex-1 text-white flex-col sm:gap-8 gap-4 relative z-30">
                                            <h1 className="font-bold  sm:text-4xl text-lg">{e.heading}</h1>
                                            <p className="">{e.description}</p>
                                            <div className="sm:flex hidden gap-8">
                                                {e.buttons && e.buttons?.length > 0 && <>
                                                    {e.buttons?.map((e) => {
                                                        if (e.withBackground) {
                                                            return <Link key={e.url.current + e.text} href={e.url.current} className="bg-base-purple min-w-[9rem] flex justify-between items-center rounded-xl px-2 text-white border-2 border-base-purple py-2 ">{e.text}<MdKeyboardDoubleArrowRight className="text-lg" /></Link>
                                                        }
                                                        return <Link key={e.url.current + e.text} href={e.url.current} className="bg-transparent min-w-[9rem] flex justify-between items-center rounded-xl px-2 text-white border-2  border-white py-2">{e.text}<MdKeyboardDoubleArrowRight className="text-lg" /></Link>
                                                    })}
                                                </>}
                                        ...
                                        <div className=" flex-1 flex flex-col gap-2">
                                            <Image src={e.imageUrl} alt="shipment" width={100} className="w-full h-full" height={100} />
            ...
            {variation == 3 && <div className={`w-full p-8 flex flex-col gap-16 items-center `}>
